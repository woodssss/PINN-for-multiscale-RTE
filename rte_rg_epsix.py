...
class stdst():
    def __init__(self, epsi_x, epsi_x_d, epsi_l, epsi_r, Nx_f, Nv_f, x_f, v_f, lx, lv, Bd_weight, Tset, Train_BC_L, Train_BC_R,
                 fL_train, fR_train, nbc, weights, dx, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr, Nx, Nv, xt, vt, dxt, wt, Test, f_ref, f_ref_vec, epsi_x_r):
        self.dtype = dtype
        ...
        self.Nx, self.Nv, self.dxt = Nx, Nv, dxt

        self.xt, self.vt,self.wt = tf.convert_to_tensor(xt, dtype=self.dtype), tf.convert_to_tensor(vt, dtype=self.dtype), tf.convert_to_tensor(wt, dtype=self.dtype)
        ...
        self.xxt, self.vvt = np.meshgrid(x, v)

        # number of layers for rho and g
        self.nl, self.nr = nl, nr

        self.epsi_l, self.epsi_r = epsi_l, epsi_r
        ...
        self.epsi_x = tf.convert_to_tensor(epsi_x, dtype=self.dtype)
        self.epsi_x_d = tf.convert_to_tensor(epsi_x_d, dtype=self.dtype)

        self.epsi_x_vec, self.epsi_x_d_vec = self.get_rho_vec(self.epsi_x), self.get_rho_vec(self.epsi_x_d)

        self.epsi_x_r = tf.convert_to_tensor(epsi_x_r, dtype=self.dtype)
        spr = tf.ones((self.Nv, 1))

        self.epsi_x_r_vec = self.Kron_TF(self.epsi_x_r, spr)
        self.epsi_x_r_vec = tf.reshape(self.epsi_x_r_vec, [self.Nx * self.Nv, 1])
    ...
    def get_nn(self):
        ...
        def myact(x):
            return tf.nn.relu(x) - 5

        def myact2(x):
            return tf.nn.relu(tf.nn.relu(x))
    ...
    def get_pde1(self):
        ...
        pde1 = self.get_intv(vg_x) * 0.5 * self.epsi_x + self.epsi_x_d*self.get_intv(self.v_train*g)*0.5
    ...
    def get_pde2(self):
        ...
        pde2 = self.v_train * rho_x_vec + self.v_train*self.epsi_x_d_vec*g + self.v_train*self.epsi_x_vec*g_x + g
    ...
    def get_f_bc_loss(self):
        ...
        fL = rhoL_vec + self.epsi_l * gL
        fR = rhoR_vec + self.epsi_r * gR

        # print('bcsp',  fL.shape, self.fL_train.shape)
        return tf.reduce_mean(tf.square(fL - 5 * tf.ones_like(fL))) + tf.reduce_mean(tf.square(fR))
        #return tf.reduce_mean(tf.square(fL - tf.zeros_like(fL))) + tf.reduce_mean(tf.square(fR))
    ...
    def get_loss(self):
        ...
        J2 = tf.reduce_mean(self.get_intv(tf.square(pde2)))*self.dx
    ...
    def get_test_error(self):
        ...
        f_pred = rho_pred_vec + self.epsi_x_r_vec*g_pred
        ...
        # print('sf', f_pred.shape, rho_pred_vec.shape, g_pred.shape, self.epsi_x_r_vec.shape)
        # asdasd
        #
    ...
    def fit(self):
        ...
        for epoch in range(self.num_ad_epochs):
            ...
            if epoch % 500 == 0:
                ...
                f_pred = rho_vec_pred + self.epsi_x_vec * g_pred
                plt.figure(1)
                ...
                with open(self.file_name, 'a') as fw:
                    print('This is epsix Nx=', self.Nx_f, file=fw)
                    print('Epoch: %d, Loss: %.3e, Time: %.2f' %
...
if __name__ == "__main__":
    ...
    # define epsi(x) function

    epsi_x = 1/(1+np.exp(cc*(x_f-0.5)))
    epsi_x_d = -cc*np.exp(cc*(x_f-0.5))/(1+np.exp(cc*(x_f-0.5)))**2

    epsi_l = 1/(1+np.exp(cc*(-0.5)))
    epsi_r = 1/(1+np.exp(cc*(0.5)))


    plt.figure(1)
    plt.plot(x_f, epsi_x,'r-o')

    plt.figure(2)
    plt.plot(x_f, epsi_x_d, 'r-o')
    ...
    for i in range(int(Nv / 2)):
        ...
        BC[i + int(Nv / 2)] = epsi_x_r[0] * v[i + int(Nv / 2)] / dxr * 5
    ...
    fname = 'rg_sigma_epsix_bd_' + str(Bd_weight) + '_cc_' + num2str_deciaml(cc) + '_nl_' + str(nl) + '_nr_' + str(nr)

    file_name = fname + '.txt'

    # define model
    mdl = stdst(epsi_x, epsi_x_d, epsi_l, epsi_r, Nx_f, Nv_f, x_f, v_f, lx, lv, Bd_weight, Tset, Train_BC_L, Train_BC_R, fL_train,
                fR_train, nbc, weights_vf, dx, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr, Nx, Nv, xt, vt, dxt, wt, Test, f_ref, f_ref_vec, epsi_x_r)
