...
class stdst():
    def __init__(self, epsi, Nx_f, Nv_f, x_f, v_f, lx, lv, Bd_weight, Tset, Train_BC_L, Train_BC_R,
                 fL_train, fR_train, nbc, weights, dx, weights_xv, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr, Nx, Nv, xt, vt, dxt, wt, Test, f_ref, f_ref_vec):
        self.dtype = dtype
        ...
        self.weights_xv = tf.convert_to_tensor(weights_xv, dtype=self.dtype)
    ...
    def get_f_bc_loss(self):
        ...
        return tf.reduce_mean(tf.square(fL - self.fL_train)) + tf.reduce_mean(tf.square(fR))

    # define loss function
    def get_loss(self):
        # pde
        pde1, pde2, pde3 = self.get_pde1(), self.get_pde2(), self.get_pde3()
    ...
    def fit(self):
        ...
        for epoch in range(self.num_ad_epochs):
            ...
            if epoch % 500 == 0:
                ...
                plt.figure(1)
                plt.plot(self.x_f, rho_pred.numpy(), 'r-o', self.xt, rho_test, 'b-*')
...
if __name__ == "__main__":
    ...
    # define training set
    # [x_f, v_f] for pde, since we need to evaluate integral, here we use quadrature rule for v_f
    # and uniform grid for x_f
    Nv_f = 80
    ...
    tmp_sk = np.ones((Nx_f, 1))

    weights_xv = np.kron(tmp_sk, weights_vf)
    ...
    for i in range(int(Nv / 2)):
        BC[i + int(Nv / 2)] = epsi * v[i + int(Nv / 2)] / dxr * 5 * np.sin(v[i + int(Nv / 2)])
    ...
    filename = 'rg_ori_bl_epsi_' + num2str_deciaml(epsi) + '_jw_' + str(Bd_weight) + '_Nx_' + str(Nx_f) + '_nl_' + str(
        nl)  + '_nr_' + str(nr)
    ...
    mdl = stdst(epsi, Nx_f, Nv_f, x_f, v_f, lx, lv, Bd_weight, Tset, Train_BC_L, Train_BC_R, fL_train,
                fR_train, nbc, weights_vf, dx, weights_xv, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr, Nx, Nv, xt, vt, dxt, wt, Test, f_ref, f_ref_vec)
    ...
    # fig = plt.figure(1)
    # ax = fig.add_subplot(111, projection='3d')
    # ax.plot_surface(xx, vv, f_pred.reshape(Nx_f, Nv_f).T)
    # plt.show()
