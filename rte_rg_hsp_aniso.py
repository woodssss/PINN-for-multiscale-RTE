...
class stdst():
    def __init__(self, epsi, Nx_f, Nv_f, x_f, x_p, v_f, lx, lv, Bd_weight, Tset, Test, Train_BC_L, Train_BC_R, fL_train, fR_train, Gamma_pred, Gamma_pred_vec, L1_M, L2_V, nbc, weights, dx, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr):
        self.dtype=dtype
...
if __name__ == "__main__":
    ...
    G_file_name = 'half_space_aniso_with_lx10' + '.h5'


    def my_act(x):
        return tf.nn.sigmoid(x) * 5 * np.sin(1)
    ...
    # save model
    file_name = 'rg_aniso_hsp_epsi_' + num2str_deciaml(epsi) + '_Nx_' + str(Nx_f) + '_nl_' + str(
        nl) + '_nr_' + str(nr)  +'.' + 'txt'

    # define model
    mdl = stdst(epsi, Nx_f, Nv_f, x_f, x_p, v_f, lx, lv, Bd_weight, Tset, Test, Train_BC_L, Train_BC_R, fL_train,
                fR_train, Gamma_pred_p, Gamma_pred_vec_p, L1_M, L2_V, nbc, weights_vf, dx, dtype, optimizer, num_ad_epochs, num_bfgs_epochs, file_name, nl, nr)
    ...
    model_name = 'rg_aniso_hsp_epsi_' + num2str_deciaml(epsi) + '_gc_' + '_Nx_' + str(Nx_f) + '_nl_' + str(
        nl) + '_nr_' + str(nr) +'.' + 'h5'
    ...
    f_pred = rho_vec_pred + epsi* g_pred + Gamma_pred_p

    print('sss', rho_vec_pred.shape, g_pred.shape, Gamma_pred.shape)

    npy_name = 'rg_aniso_hsp_epsi_' + num2str_deciaml(epsi) + '_Nx_' + str(Nx_f) + '_nl_' + str(
        nl) + '_nr_' + str(nr) + '.' + 'npy'
